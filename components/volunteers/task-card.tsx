  ...
  return (
    <article className="rounded-lg border border-border bg-card p-4 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-medium">{request?.address || `Rescue task ${task.id.slice(0, 8)}`}</h2>
      ...
      {task.status === 'accepted' && verified && (
        <div className="space-y-2 border-t border-border pt-4">
          ...
          <Textarea id={`note-${task.id}`} rows={2} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Example: Travelling to the affected area"  />
          <Button onClick={() => onStart(note)} disabled={busy} >{busy ? 'Updating...' : 'Start Rescue Work'}</Button>
        </div>
      )}

      {task.status === 'in_progress' && verified && (
        <div className="flex items-center justify-between gap-3 border-t border-border pt-4">
          <p className="text-xs text-muted-foreground">{task.progress_note || 'Rescue work is in progress.'}</p>
          <Button onClick={onComplete} disabled={busy} className="shrink-0">{busy ? 'Completing...' : 'Complete Task'}</Button>
        </div>
      )}

      {task.status === 'completed' && (
        <p className="border-t border-border pt-4 text-xs text-primary">Completed {task.completed_at ? new Date(task.completed_at).toLocaleString() : ''}</p>
      )}
    </article>
  );
};
