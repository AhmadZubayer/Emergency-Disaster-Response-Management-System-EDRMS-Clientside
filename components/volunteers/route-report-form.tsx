...
import { routeReportSchema } from '@/lib/validations/volunteer-field-report-schema';
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <MuiSelect
            id="routeType"
            value={reportType}
            onChange={(val) => setReportType(val as 'blocked_route' | 'dangerous_route')}
            options={[
              { label: 'Blocked route', value: 'blocked_route' },
              { label: 'Dangerous route', value: 'dangerous_route' },
            ]}
          />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="routeSeverity">Severity</Label>
          <MuiSelect
            id="routeSeverity"
            value={severity}
      ...
      <div className="space-y-1.5">
        ...
        <Textarea id="routeDescription" value={description} onChange={(event) => setDescription(event.target.value)} rows={3} required placeholder="Explain the obstruction or danger..." />
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label htmlFor="routeLatitude">Latitude *</Label>
          <Input id="routeLatitude" type="number" step="any" value={latitude} onChange={(event) => setLatitude(event.target.value)} required />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="routeLongitude">Longitude *</Label>
          <Input id="routeLongitude" type="number" step="any" value={longitude} onChange={(event) => setLongitude(event.target.value)} required />
        </div>
      </div>
      <div className="space-y-1.5">
