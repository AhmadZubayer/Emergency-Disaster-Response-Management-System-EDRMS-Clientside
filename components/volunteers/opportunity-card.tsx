  ...
  return (
    <article className="rounded-lg border border-border bg-card p-4 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-medium">{opportunity.title}</h2>
          <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
            <Building2 className="size-3.5" />
            {opportunity.organization_user?.name || 'Relief organization'}
          </p>
        </div>
        <Badge
          variant={applicationStatus === 'approved' ? 'default' : 'secondary'}
        >
          {statusLabel}
        </Badge>
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
        ...
        <span className="flex items-center gap-1.5">
          ...
          {opportunity.needed_volunteers} needed
        </span>
      </div>
      {opportunity.disaster_name && (
        <p className="text-xs">
